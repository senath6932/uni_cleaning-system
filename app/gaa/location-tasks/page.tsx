...
import {
  listLocationTaskReferenceData,
  listManagedLocationTasks,
  type LocationTaskListResult,
  type LocationTaskReferenceData,
} from "@/lib/location-task-management";
import { GaaLocationTaskManagementClient } from "./location-task-management-client";
...
export default async function GaaLocationTasksPage({
  searchParams,
}: {
      ...
      <main className="flex min-h-screen items-center justify-center bg-slate-950 px-6 text-slate-100">
        <div className="max-w-xl rounded-3xl border border-red-500/30 bg-red-500/10 p-8">
          ...
          <p className="mt-3 text-sm leading-7 text-slate-300">
            Only GAA users can manage location task assignments.
          </p>
        </div>
      </main>
    );
  ...
  const filters = {
    ...
    sortBy:
      sortBy === "createdAt" || sortBy === "allocatedAmount" || sortBy === "active"
        ? sortBy
        : "createdAt",
    sortOrder: sortOrder === "asc" ? "asc" : "desc",
  } as const;

  let data: LocationTaskListResult = {
    locationTasks: [],
    total: 0,
    page: filters.page,
  ...
  let referenceData: LocationTaskReferenceData = { locations: [], tasks: [] };
  let pageError: string | null = null;

  const [assignmentsResult, referenceResult] = await Promise.allSettled([
    listManagedLocationTasks(prisma, filters),
    listLocationTaskReferenceData(prisma),
  ]);

  if (assignmentsResult.status === "fulfilled") {
    data = assignmentsResult.value;
  } else {
    pageError = "Unable to load location task assignments right now.";
  }
  ...
  return (
    <AppShell user={appUser}>
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-6">
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.3em] text-slate-500">
                GAA Location Task Management
              </p>
              <h1 className="mt-2 text-3xl font-semibold text-slate-950">
                Assign cleaning tasks to locations
              </h1>
              <p className="mt-2 max-w-3xl text-sm leading-7 text-slate-600">
                Configure which reusable cleaning tasks belong to each active campus location and
                set the location-specific allocated amount.
              </p>
            </div>
        ...
        <GaaLocationTaskManagementClient
          initialData={data}
