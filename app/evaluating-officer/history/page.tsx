...
import { listOfficerEvaluationHistory } from "@/lib/evaluating-officer-monitoring";

export default async function EvaluatingOfficerHistoryPage() {
      ...
      <main className="flex min-h-screen items-center justify-center bg-slate-950 px-6 text-slate-100">
        <div className="max-w-xl rounded-3xl border border-red-500/30 bg-red-500/10 p-8">
          ...
          <p className="mt-3 text-sm leading-7 text-slate-300">
            This history area is restricted to assigned Evaluating Officers.
          </p>
        </div>
      </main>
    );
  ...
  const history = await listOfficerEvaluationHistory(prisma, appUser.id);
                ...
                {history.length === 0 ? (
                  ...
                  history.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50">
                      <td className="px-4 py-4 text-sm text-slate-600">{item.evaluationDate}</td>
                      <td className="px-4 py-4 text-sm text-slate-900">
                        <p className="font-medium">{item.location.code}</p>
                        <p className="text-slate-500">{item.location.name}</p>
                      </td>
                      <td className="px-4 py-4 text-sm text-slate-600">{item.task.name}</td>
                      <td className="px-4 py-4 text-sm text-slate-600">{item.task.frequency}</td>
                      <td className="px-4 py-4 text-sm text-slate-600">{item.result}</td>
